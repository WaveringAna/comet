...
impl InstanceLock {
                ...
                match errno.raw_os_error() {
                    ...
                    Some(libc::EWOULDBLOCK) => {
                        ...
                        return Err(EngineError::Other(format!(
                            "another nova engine is already running on {} (pid {}); \
                             stop it or use a different data dir (NOVA_DATA_DIR)",
                            data_dir.display(),
    ...
    /// cannot test a lock without taking it). Used by `nova status` and the
