//! Uploads — attachment staging and device-local serving
//! (feature-inventory §3.7 "Uploads"; port of comet's `uploads.ts`).
//!
//! The UI streams a file as base64 chunks (~60KB, sized for direct peer RPC when the
...
impl Uploads {
    ...
    /// path. Remote readers request chunks directly from this device over Nova.
