//! nova-update — release checking and self-update, shared by the engine (the
//! background checker + `ApplyUpdate`), the CLI (`nova update`), and the UI
...
//! - **Managed** (`~/.nova-native/app/<ver>` + `current` symlink — the curl|sh
...
//!   natively.
//! - **MacApp** (running out of `Nova.app`): download the app tarball, swap the
...
/// `nova-<ver>-<os>-<arch>.tar.gz` — the headless/CLI tarball (Linux CI builds).
pub fn headless_artifact(version: &str) -> String {
    let (os, arch) = platform_key();
    format!("nova-{version}-{os}-{arch}.tar.gz")
}

/// `nova-<ver>-macos-<arch>-app.tar.gz` — the packaged `Nova.app` bundle.
pub fn mac_app_artifact(version: &str) -> String {
    let (_, arch) = platform_key();
    format!("nova-{version}-macos-{arch}-app.tar.gz")
}
...
fn http_client() -> anyhow::Result<reqwest::Client> {
    reqwest::Client::builder()
        .user_agent(concat!("nova/", env!("CARGO_PKG_VERSION")))
        .build()
...
pub enum InstallKind {
    /// `~/.nova-native/app/<ver>/nova` behind the `current` symlink
    /// (curl|sh installer / a previous `nova update`).
...
fn detect_install_from(exe: &Path, home: Option<&Path>) -> InstallKind {
    if let Some(home) = home {
        ...
        for data_dir in [".nova-native", ".comet-native"] {
            let app_root = home.join(data_dir).join("app");
...
pub async fn stage_headless(
    ...
    if dest.join("nova").exists() {
        ensure_legacy_binary_alias(app_root, &dest)?;
        return Ok(dest);
    }
    ...
    let result = async {
        ...
        if !unpacked.join("nova").is_file() {
            bail!("tarball {file} did not contain a nova binary");
        }
        // A pre-rename service still launches `app/current/comet`. Keep that
        // stable while installing Nova into its legacy managed root; fresh
        // installs expose only the new binary name.
        ensure_legacy_binary_alias(app_root, &unpacked)?;
        match std::fs::rename(&unpacked, &dest) {
            Ok(()) => {}
            // Lost a race with another stager — the staged copy is equivalent.
            Err(_) if dest.join("nova").exists() => {}
            Err(err) => {
...
/// Restart the installed engine service (the same units `nova daemon` and the
/// curl|sh installer manage). Called after a symlink swap so the running daemon
/// picks up the new binary.
pub fn restart_service(app_root: &Path) -> anyhow::Result<()> {
    let legacy = is_legacy_app_root(app_root);
    if cfg!(target_os = "macos") {
        let output = std::process::Command::new("id").arg("-u").output()?;
        let uid = String::from_utf8_lossy(&output.stdout).trim().to_string();
        let (primary, fallback) = if legacy {
            ("sh.zeron.comet", "sh.zeron.nova")
        } else {
            ("sh.zeron.nova", "sh.zeron.comet")
        };
        run(
            "launchctl",
            &["kickstart", "-k", &format!("gui/{uid}/{primary}")],
        )
        .or_else(|_| {
            run(
                "launchctl",
                &["kickstart", "-k", &format!("gui/{uid}/{fallback}")],
            )
        })
    } else {
        let (primary, fallback) = if legacy {
            ("comet-native.service", "nova-native.service")
        } else {
            ("nova-native.service", "comet-native.service")
        };
        run("systemctl", &["--user", "restart", primary])
            .or_else(|_| run("systemctl", &["--user", "restart", fallback]))
    }
}

fn is_legacy_app_root(app_root: &Path) -> bool {
    app_root
        .parent()
        .and_then(Path::file_name)
        .is_some_and(|name| name == ".comet-native")
}

fn ensure_legacy_binary_alias(app_root: &Path, version_dir: &Path) -> anyhow::Result<()> {
    if !is_legacy_app_root(app_root) || version_dir.join("comet").exists() {
        return Ok(());
    }
    #[cfg(unix)]
    std::os::unix::fs::symlink("nova", version_dir.join("comet"))
        .context("creating legacy comet binary alias")?;
    Ok(())
}
...
/// Download + unpack the app tarball into `{data_dir}/updates/<ver>/Nova.app`
...
pub async fn stage_mac_app(
    ...
    let staged = dir.join("Nova.app");
    if staged.join("Contents/MacOS/nova").exists() {
        return Ok(staged);
    }
    ...
    if !staged.join("Contents/MacOS/nova").exists() {
        bail!("app tarball {file} did not contain Nova.app");
    }
    Ok(staged)
}
...
/// `NOVA_AUTO_UPDATE=1|true|yes` — headless daemons apply updates themselves.
fn auto_update_enabled() -> bool {
    std::env::var("NOVA_AUTO_UPDATE")
        .or_else(|_| std::env::var("COMET_AUTO_UPDATE"))
...
/// stream). Managed installs with `NOVA_AUTO_UPDATE` set stage + apply + service
...
impl Updater {
    ...
    pub async fn apply(&self) -> anyhow::Result<String> {
        ...
        tokio::spawn(async move {
            ...
            if let Err(err) = restart_service(&app_root) {
...
mod tests {
    ...
    fn install_kind_detection() {
        assert_eq!(
            detect_install_from(
                Path::new("/home/u/.nova-native/app/0.1.1/nova"),
                Some(Path::new("/home/u")),
            ),
            InstallKind::Managed {
                app_root: PathBuf::from("/home/u/.nova-native/app")
            }
        );
        assert_eq!(
            detect_install_from(
        ...
        assert_eq!(
            detect_install_from(
                Path::new("/Applications/Nova.app/Contents/MacOS/nova"),
                Some(Path::new("/Users/u")),
            ),
            InstallKind::MacApp {
                bundle: PathBuf::from("/Applications/Nova.app")
            }
        );
        // A path merely containing `.app` without the bundle layout is not a bundle.
        assert_eq!(
            detect_install_from(Path::new("/tmp/foo.app/nova"), None),
            InstallKind::Unmanaged
        );
        assert_eq!(
            detect_install_from(
                Path::new("/src/target/release/nova"),
                Some(Path::new("/home/u"))
            ),
            InstallKind::Unmanaged
        );
    ...
    fn artifact_names_match_packaging() {
        let (os, arch) = platform_key();
        assert!(headless_artifact("0.2.0").starts_with("nova-0.2.0-"));
        assert_eq!(
            headless_artifact("0.2.0"),
            format!("nova-0.2.0-{os}-{arch}.tar.gz")
        );
    ...
    fn manifest_parses_with_and_without_files() {
        let full: Manifest = serde_json::from_str(
            r#"{"version":"0.1.1","files":{"nova-0.1.1-linux-x86_64.tar.gz":{"sha256":"abc"}}}"#,
        )
        .unwrap();
        assert_eq!(full.version, "0.1.1");
        assert_eq!(
            full.files["nova-0.1.1-linux-x86_64.tar.gz"]
                .sha256
                .as_deref(),
            Some("abc")
        );
    ...
    fn headless_symlink_swap() {
        ...
        for ver in ["0.1.0", "0.1.1"] {
            ...
            std::fs::write(app_root.join(ver).join("nova"), ver).unwrap();
        }
        ...
        let legacy_root = tmp.path().join(".comet-native/app");
        let legacy_version = legacy_root.join("0.2.0");
        std::fs::create_dir_all(&legacy_version).unwrap();
        std::fs::write(legacy_version.join("nova"), "0.2.0").unwrap();
        ensure_legacy_binary_alias(&legacy_root, &legacy_version).unwrap();
        assert_eq!(
            std::fs::read_link(legacy_version.join("comet")).unwrap(),
            PathBuf::from("nova")
        );
    }
}
