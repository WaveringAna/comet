//! Animation kit — the nova motion catalog as reusable helpers over gpui
...
//! - `nova-pulse` 2.4s staggered cell opacity 0.08→1, scale 0.9→1 (loaders)
...
/// (`transition-colors` et al. carry it unless overridden; nova never does).
...
/// Nova loader pulse period: 2.4s.
pub const NOVA_PULSE: MotionSpec = MotionSpec::new(2400, EASE);
...
/// (nova also scales 0.96→1; divs have no scale transform in gpui — approximated.)
...
/// Nova-pulse floor opacity.
// The loader curves are shared with the terminal viewport
// (`nova_proto::motion`): a loading indicator is a brand surface, and two of
// them that disagree read as two products. The constants and the math live
// there; this crate animates them with gpui, `nova-tui` with cell colours.
pub use nova_proto::motion::{
...
// the pointer enters. The original nova puts Tailwind `transition-colors`
...
/// Dev/measurement knob (`NOVA_MOTION_SCALE`, default 1): stretches every
/// catalog timeline by this factor — e.g. `NOVA_MOTION_SCALE=10` slows the
...
mod tests {
    ...
    fn catalog_timings_match_nova() {
        ...
        assert_eq!(NOVA_PULSE.duration_ms, 2400);
