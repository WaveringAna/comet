...
//!
//! nova never called `cx.set_menus`, so on macOS `NSApp.mainMenu` stayed nil:
...
pub fn init(cx: &mut App) {
    ...
    // Window verbs route to the active window. nova is single-window, so a
...
/// ⌘Q / "Quit Nova". `cx.quit()` runs the platform's standard quit routine,
...
/// The nova menu bar. macOS renders this natively; mac-only entries are gated
...
pub fn app_menus() -> Vec<Menu> {
    ...
    let mut app_items = vec![
        ...
        MenuItem::action("About Nova", About).disabled(true),
        MenuItem::separator(),
    ];
    if macos {
        app_items.extend([
            ...
            MenuItem::separator(),
            MenuItem::action("Hide Nova", Hide),
            MenuItem::action("Hide Others", HideOthers),
            MenuItem::action("Show All", ShowAll),
            MenuItem::separator(),
        ]);
    }
    app_items.push(MenuItem::action("Quit Nova", Quit));

    let mut menus = vec![
        Menu::new("Nova").items(app_items),
