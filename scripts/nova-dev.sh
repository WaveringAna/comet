...
# `cargo build -p nova`, and only after the NEW window reports
...
#
# Env overrides: NOVA_DATA_DIR (default ~/.nova-native), NOVA_IPC_PORT
# (default 27654, matching apps/nova), NOVA_DEV_READY_TIMEOUT (seconds, 30).
set -euo pipefail
cd "$(dirname "$0")/.."

if [[ -z "${NOVA_DATA_DIR:-}" ]]; then
  if [[ -n "${COMET_DATA_DIR:-}" ]]; then
    NOVA_DATA_DIR="$COMET_DATA_DIR"
  elif [[ ! -e "$HOME/.nova-native" && -e "$HOME/.comet-native" ]]; then
    NOVA_DATA_DIR="$HOME/.comet-native"
  else
    NOVA_DATA_DIR="$HOME/.nova-native"
  fi
fi
export NOVA_DATA_DIR
export NOVA_IPC_PORT="${NOVA_IPC_PORT:-${COMET_IPC_PORT:-27654}}"
...
if ! (echo >"/dev/tcp/127.0.0.1/$NOVA_IPC_PORT") 2>/dev/null; then
  echo "▸ starting engine daemon on :$NOVA_IPC_PORT (data: $NOVA_DATA_DIR)"
  cargo build -p nova -q
  ./target/debug/nova headless >"$RUN_DIR/daemon.log" 2>&1 &
  for _ in $(seq 1 60); do
    (echo >"/dev/tcp/127.0.0.1/$NOVA_IPC_PORT") 2>/dev/null && break
    sleep 0.5
  done
fi
...
spawn_gui() {
  ...
  : >"$log"
  NOVA_HOTRELOAD=1 ./target/debug/nova >"$log" 2>&1 &
  GUI_PID=$!
}
...
cargo build -p nova -q
spawn_gui
echo "▸ gui gen $GEN up (pid $GUI_PID) — editing crates/ui or apps/nova swaps it in place"
...
while true; do
  ...
  if ! cargo build -p nova 2>&1 | tail -5; then
